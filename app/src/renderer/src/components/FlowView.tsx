...
function Str({ s }: { s: string }): React.JSX.Element {
  ...
  return (
    <>
      ...
      <button className="ml-1 border-0 bg-transparent p-0 font-mono text-[0.625rem] text-foreground-subtlest underline" onClick={() => setOpen((o) => !o)}>
  ...
  if (!removed.length && !added.length && !changed.length) {
    return (
      <details className="my-0.5 min-w-0">
        <summary className="cursor-pointer list-none font-mono text-[0.6875rem] text-foreground-subtlest">= same as <span className="text-foreground-subtle">{vs ?? 'the input'}</span> ▸</summary>
        <ObjView j={to} />
      </details>
    )
  }
  return (
    <div className="my-0.5 flex min-w-0 flex-col gap-0.5 rounded-md border border-border bg-panel px-2 py-1">
      ...
      <details className="min-w-0">
        <summary className="cursor-pointer list-none text-[0.625rem] text-foreground-subtlest">full object ▸</summary>
        <ObjView j={to} />
      </details>
    </div>
  )
...
function ObjView({ j, badge, childAnc, depth = 0 }: { j: Record<string, unknown> | unknown[]; badge?: string; childAnc?: Record<string, AncestorDelta>; depth?: number }): React.JSX.Element {
  ...
  return (
    <div className="my-0.5 flex min-w-0 flex-col gap-px rounded-md border border-border bg-panel px-2 py-1">
      ...
      {entries.length > 24 && <span className="text-[0.625rem] text-foreground-subtlest">…+{entries.length - 24} more</span>}
    </div>
  )
}
...
function StructVal({ j, occ, badge }: { j: Record<string, unknown> | unknown[]; occ?: Occ; badge?: string }): React.JSX.Element {
  if (occ && !occ.first) {
    return (
      <details className="my-0.5 min-w-0">
        <summary className="cursor-pointer list-none font-mono text-[0.6875rem] text-foreground-subtlest">
...
function Val({ v, occ }: { v: TraceValue; occ?: Occ }): React.JSX.Element {
  ...
  if (j) {
    return (
      <div className="flex w-full min-w-0 items-start gap-1.5">
        <span className="shrink-0 pt-1 font-mono text-[0.75rem] text-foreground-subtlest">{v.name}</span>
  ...
  return (
    <span className="inline-flex max-w-full items-baseline gap-1">
      <span className="font-mono text-[0.75rem] text-foreground-subtlest">{v.name}</span>
      <span className="min-w-0 break-all font-mono text-[0.75rem] text-foreground"><Str s={v.repr} /></span>
    </span>
  )
}
      ...
      <div className="flex items-start gap-2 py-1">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="font-mono text-[0.8125rem] font-medium text-foreground">{frame.fn}</span>
            {frame.file && (
              <button className="font-mono text-[0.6875rem] text-foreground-subtlest underline underline-offset-2 transition-colors hover:text-foreground" onClick={() => onOpenSource?.(frame.file, frame.line)} title="open source">
                {frame.file}{frame.line ? `:${frame.line}` : ''}
              </button>
            )}
            {frame.durMs > 0 && <span className="text-[0.6875rem] text-foreground-subtlest">{frame.durMs.toFixed(1)}ms</span>}
          </div>
          <div className="flex flex-wrap items-center gap-1.5 py-0.5 font-mono text-[0.75rem]">
          ...
          {open && threw && <div className="mt-1 text-[0.8125rem] text-foreground-subtle">? {frame.fn} threw {threw.type}: {threw.message} — intended?</div>}
        </div>
      </div>
...
const headEntry = 'min-w-0 break-all font-mono text-[0.875rem] font-semibold text-foreground'
const headLang = 'shrink-0 whitespace-nowrap rounded-full border border-border px-2 text-[0.625rem] uppercase text-foreground-subtlest'
...
const headChannel = 'max-w-[300px] truncate whitespace-nowrap rounded-md border border-border px-2 py-px text-[0.625rem] uppercase tracking-wide text-foreground-subtlest'
const headMeta = 'shrink-0 whitespace-nowrap text-[0.6875rem] text-foreground-subtlest'
const footCls = 'mt-auto pt-3 text-[0.75rem] leading-relaxed text-foreground-subtlest'
const qCls = 'mt-1 text-[0.8125rem] text-foreground-subtle'
...
export function FlowView({ trace, onOpenSource }: { trace: TraceDoc; onOpenSource?: (file: string, line: number | null) => void }): React.JSX.Element {
    ...
    return (
      <div className={shellCls}>
        ...
        <div className={unobsBox}>
          <div className="text-[0.875rem] font-medium text-foreground">grasp couldn&rsquo;t observe this run</div>
          <div className="mt-1 text-[0.8125rem] text-foreground-subtle">{trace.unobservable?.reason}</div>
          {trace.unobservable?.hint && <div className="mt-0.5 text-[0.75rem] text-foreground-subtlest">{trace.unobservable.hint}</div>}
          <div className="mt-3 text-[0.75rem] leading-relaxed text-foreground-subtlest">This is a tooling gap, not the code&rsquo;s behavior — grasp will not invent a flow it didn&rsquo;t measure.</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={shellCls}>
      ...
      {trace.how.length > 90 ? (
        <details className="text-[0.75rem] text-foreground-subtlest">
        ...
        <div className="text-[0.75rem] text-foreground-subtlest">ran: {trace.how}</div>
      )}
      <div className="flex flex-col">
        {roots.map((r) => (<Frame key={r.id} frame={r} children={byParent.get(r.id) ?? []} byParent={byParent} onOpenSource={onOpenSource} occ={occ} />))}
      </div>
      {plumbingCount > 0 && (
        <button className="self-start rounded-md border border-border bg-transparent px-2.5 py-1 text-[0.75rem] text-foreground-subtlest transition-colors hover:bg-surface-hover hover:text-foreground" onClick={() => setShowPlumbing((v) => !v)}>
      ...
      {(trace.stdout || trace.stderr) && (
        <details className="mt-2 rounded-lg border border-border bg-panel p-3">
          <summary className="cursor-pointer text-[0.75rem] text-foreground-subtle">observed output</summary>
          {trace.stdout && <pre className="mt-2 max-h-[120px] overflow-auto whitespace-pre-wrap font-mono text-[0.75rem] text-foreground-subtle">{trace.stdout}</pre>}
          {trace.stderr && <pre className="mt-1 max-h-[120px] overflow-auto whitespace-pre-wrap font-mono text-[0.75rem] text-destructive">{trace.stderr}</pre>}
        </details>
      )}
  ...
  return (
    <div style={{ marginLeft: `${f.depth * 16}px` }}>
      <div className="flex items-start gap-2 py-1">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className={`rounded px-1.5 text-[0.625rem] uppercase ${statusColor} ${d.status === 'changed' ? 'bg-tag' : d.status === 'added' ? 'bg-tag' : d.status === 'removed' ? 'bg-destructive/10' : 'bg-surface'}`}>{d.status}</span>
            <span className="font-mono text-[0.8125rem] font-medium text-foreground">{f.fn}</span>
            {f.file && <button className="font-mono text-[0.6875rem] text-foreground-subtlest underline underline-offset-2 hover:text-foreground" onClick={() => onOpenSource?.(f.file, f.line)}>{f.file}{f.line ? `:${f.line}` : ''}</button>}
          </div>
          {d.status === 'changed' ? (
            <div className="flex flex-col gap-0.5 py-0.5">
              {d.changes.map((c, i) => (
                <div key={i} className="flex items-center gap-1.5 font-mono text-[0.75rem]">
...
export function FlowDiffView({ diff, onOpenSource }: { diff: TraceDiff; onOpenSource?: (file: string, line: number | null) => void }): React.JSX.Element {
  ...
  return (
    <div className={shellCls}>
      ...
      {diff.rootChange && (
        <div className="rounded-lg border border-border bg-surface px-3 py-2 font-mono text-[0.8125rem]">
      ...
      {diff.empty ? (
        <div className="rounded-xl border border-border bg-surface p-4">
          <div className="text-[0.875rem] font-medium text-foreground">Same flow for this input.</div>
          <div className="mt-1 text-[0.75rem] leading-relaxed text-foreground-subtlest">Both versions ran the same call tree with the same values. That is not a pass — it is one observed path.</div>
        </div>
      ) : (
        <>
          <div className="flex flex-col">{rows.map((d, i) => <DeltaRow key={i} d={d} onOpenSource={onOpenSource} />)}</div>
          <button className="self-start rounded-md border border-border px-2.5 py-1 text-[0.75rem] text-foreground-subtlest transition-colors hover:bg-surface-hover" onClick={() => setShowUnchanged((v) => !v)}>
          ...
          {diff.questions.length > 0 && (
            <div className="rounded-lg border border-border bg-surface p-3">
              <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">You adjudicate</div>
...
export function IntroView({ intro, onPickFlow }: { intro: IntroDoc; onPickFlow?: (name: string) => void }): React.JSX.Element {
  ...
  return (
    <div className={shellCls}>
      ...
      <div className="rounded-xl border border-border bg-surface p-3 text-[0.8125rem] leading-relaxed text-foreground">{intro.how}</div>
      <div className="rounded-xl border border-border bg-surface p-3">
        <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">flows you can ask to see</div>
        <div className="mt-1 flex flex-col">
          {intro.flows.map((f, i) => (
            <button key={i} className="flex items-baseline gap-2 border-t border-border bg-transparent px-1 py-1.5 text-left first:border-t-0 hover:bg-surface-hover" onClick={() => onPickFlow?.(f.name)}>
              <span className="text-[0.8125rem] font-medium text-foreground">{f.name}</span>
              <span className="min-w-0 text-[0.75rem] text-foreground-subtle">{f.what}</span>
              <span className="ml-auto shrink-0 text-[0.6875rem] text-foreground-subtlest">show ▸</span>
            </button>
          ))}
        </div>
      </div>
      <div className="rounded-xl border border-border bg-surface p-3">
        <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">rules in your model</div>
        {intro.rules.length === 0 ? (
          <div className="py-1 text-[0.75rem] text-foreground-subtlest">none found at {intro.workspace}/.grasp/model.yaml — state an always/never in chat and it will be staged for your signature there</div>
        ) : (
          <div className="mt-1 flex flex-col">
            {intro.rules.map((r) => (
              <div key={r.id} className="flex items-baseline gap-2 border-t border-border py-1.5 first:border-t-0">
                <span className="font-mono text-[0.6875rem] text-foreground-subtlest">{r.id}</span>
                <span className="min-w-0 text-[0.8125rem] text-foreground">{r.text}</span>
                <span className={`ml-auto shrink-0 font-mono text-[0.6875rem] ${chip(r.status)}`}>{r.status}</span>
              </div>
            ))}
          </div>
        )}
        {intro.rules.some((r) => r.status === 'staged') && (
          <div className="mt-1.5 text-[0.6875rem] leading-relaxed text-foreground-subtlest">staged = harvested from your docs, awaiting you: delete its staged flag in .grasp/model.yaml to enforce, or delete the rule.</div>
        )}
      </div>
      <div className="rounded-lg border border-border bg-surface px-3 py-2 text-[0.8125rem] text-foreground-subtle">next: {intro.suggestion}</div>
    </div>
  )
}
...
function ReportView({ report, onAdjudicate }: { report: ModelReport; onAdjudicate?: (prompt: string) => void }): React.JSX.Element {
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-3">
      <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">
      ...
      <div className="mt-1.5 flex flex-col">
        {report.rows.filter((r) => !r.note?.startsWith('staged')).map((r) => (
          <div key={r.id} className="border-t border-border py-1.5 first:border-t-0">
            <div className="flex items-start gap-2">
              <div className="min-w-0 flex-1">
                <span className="mr-2 font-mono text-[0.6875rem] text-foreground-subtlest">{r.id}</span>
                <span className="text-[0.8125rem] text-foreground">{r.text}</span>
              </div>
              <span className={`shrink-0 font-mono text-[0.75rem] ${statusCls(r.status)}`}>
                {r.status}
                {r.status !== 'untested' && ` (${r.covered} case${r.covered === 1 ? '' : 's'})`}
              </span>
            </div>
            <div className="text-[0.6875rem] text-foreground-subtlest">
            ...
            {r.counterexamples.length > 0 && (
              <div className="mt-1 flex flex-col gap-0.5">
                {r.counterexamples.map((c, i) => (
                  <div key={i} className="font-mono text-[0.75rem] text-foreground-subtle">
                    {c.label ?? JSON.stringify(c.input)} — observed: {c.observed}
                  </div>
                ))}
                {onAdjudicate && (
                  <div className="mt-0.5 flex gap-1.5">
                    <button className="rounded-md border border-border px-2 py-0.5 text-[0.6875rem] text-foreground-subtle hover:bg-surface-hover" onClick={() => onAdjudicate(`Rule ${r.id} ("${r.text}") is violated — see counterexample "${r.counterexamples[0]?.label ?? 'above'}". Repair the code to satisfy the rule and re-verify with grasp_fuzz_diff.`)}>repair the code ▸</button>
                    <button className="rounded-md border border-border px-2 py-0.5 text-[0.6875rem] text-foreground-subtlest hover:bg-surface-hover" onClick={() => onAdjudicate(`Rule ${r.id} ("${r.text}") no longer reflects my intent. Based on the observed behavior, propose an amended rule text staged for my confirmation, and leave the code as is.`)}>amend the rule ▸</button>
                  </div>
                )}
              </div>
            )}
        ...
        {report.rows.length === 0 && <div className="py-1 text-[0.75rem] text-foreground-subtlest">model has no rules yet</div>}
        {report.rows.some((r) => r.note?.startsWith('staged')) && (
          <details className="border-t border-border py-1.5">
            <summary className="cursor-pointer list-none text-[0.75rem] text-foreground-subtle">
            ...
            <div className="mt-1 flex flex-col gap-0.5">
              {report.rows.filter((r) => r.note?.startsWith('staged')).map((r) => (
                <div key={r.id} className="flex min-w-0 items-baseline gap-2">
                  <span className="shrink-0 font-mono text-[0.6875rem] text-foreground-subtlest">{r.id}</span>
                  <span className="min-w-0 truncate text-[0.75rem] text-foreground-subtle">{r.text}</span>
                </div>
              ))}
            </div>
          </details>
      ...
      {report.novel.length > 0 && (
        <div className="mt-2 border-t border-border pt-2">
          <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">novel — no rule covers these</div>
            ...
            <div key={i}>
              ...
              {onAdjudicate && (
                <div className="mb-1 mt-0.5 flex gap-1.5">
                  <button className="rounded-md border border-border px-2 py-0.5 text-[0.6875rem] text-foreground-subtle hover:bg-surface-hover" onClick={() => onAdjudicate(`The novel behavior "${n.label ?? JSON.stringify(n.input)}" (${n.change}) is INTENDED. Stage it into .grasp/model.yaml as a ratified rule per the behavior-model skill, with this fuzz run as evidence.`)}>intended — remember it ▸</button>
                  <button className="rounded-md border border-border px-2 py-0.5 text-[0.6875rem] text-foreground-subtlest hover:bg-surface-hover" onClick={() => onAdjudicate(`The novel behavior "${n.label ?? JSON.stringify(n.input)}" (${n.change}) is NOT intended. Repair the code so it no longer occurs, re-verify with grasp_fuzz_diff, and propose a staged rule preventing regression.`)}>not intended — repair ▸</button>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="mt-2 text-[0.6875rem] leading-relaxed text-foreground-subtlest">{report.scope}</div>
    </div>
  )
}
...
export function FuzzDiffView({ fuzz, onOpenSource, onAdjudicate }: { fuzz: FuzzDiff; onOpenSource?: (file: string, line: number | null) => void; onAdjudicate?: (prompt: string) => void }): React.JSX.Element {
  ...
  return (
    <div className={shellCls}>
      ...
      {fuzz.claim && (
        <div className="rounded-xl border border-border bg-surface p-3">
          <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">behavioral delta — agent proposed · grasp checked</div>
          <div className="mt-1.5 font-mono text-[0.8125rem] text-foreground">
            where {fuzz.claim.rendered}
            {fuzz.claim.effectRendered && <span className="text-foreground-subtle">: {fuzz.claim.effectRendered}</span>}
          </div>
          <div className="mt-1 text-[0.75rem] leading-relaxed text-foreground-subtle">{fuzz.claim.summary}</div>
          {fuzz.claim.mismatches.length > 0 && (
            <div className="mt-1.5 flex flex-col gap-0.5 border-t border-border pt-1.5">
              ...
              {fuzz.claim.mismatchCount > fuzz.claim.mismatches.length && (
                <div className="text-[0.6875rem] text-foreground-subtlest">+{fuzz.claim.mismatchCount - fuzz.claim.mismatches.length} more mismatches</div>
              )}
            </div>
          )}
        </div>
      ...
      {fuzz.axes && (fuzz.axes.varied.length > 0 || fuzz.axes.held.length > 0) && (
        <div className="flex flex-wrap items-center gap-1.5">
          {fuzz.axes.varied.length > 0 && <span className="text-[0.6875rem] text-foreground-subtlest">varied</span>}
          {fuzz.axes.varied.map((a, i) => (
            <span key={`v${i}`} className="rounded-full border border-border px-2 py-0.5 font-mono text-[0.6875rem] text-foreground-subtle">
              {a.path}
              {a.note ? ` ${a.note}` : ''} · {a.distinct} observed
            </span>
          ))}
          {fuzz.axes.held.length > 0 && <span className="text-[0.6875rem] text-foreground-subtlest">held constant</span>}
          {fuzz.axes.held.map((h, i) => (
            <span key={`h${i}`} className="rounded-full border border-dashed border-border px-2 py-0.5 font-mono text-[0.6875rem] text-foreground-subtlest">
              {h.path} = {h.repr}
      ...
      {fuzz.axes && fuzz.axes.issues.length > 0 && (
        <div className="flex flex-col gap-0.5 text-[0.75rem] text-foreground-subtle">
      ...
      {!fuzz.report && (
        <div className="rounded-lg bg-surface px-3 py-1.5 text-[0.8125rem] text-foreground-subtle">{fuzz.scope}</div>
      )}
      {fuzz.outcomes.length > 0 && (
        <div className="rounded-xl border border-border bg-surface p-3">
          <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-foreground-subtlest">scenarios — outcomes in domain words · frames are the drill-down</div>
          {[...new Set(fuzz.outcomes.map((o) => o.scenario ?? '(unnamed scenario)'))].map((sc) => {
            ...
            return (
              <div key={sc} className="mt-1.5 border-t border-border pt-1.5 first:mt-0 first:border-t-0 first:pt-0">
                <div className="font-mono text-[0.6875rem] text-foreground-subtlest">{sc}</div>
                {changed.map((o, i) => {
                  ...
                  return (
                    <details key={i} className="py-0.5">
                      <summary className="flex cursor-pointer list-none flex-wrap items-baseline gap-x-2 gap-y-0.5">
                        <span className="text-[0.8125rem] text-foreground">{o.label ?? JSON.stringify(o.input).slice(0, 60)}</span>
                        <span className="font-mono text-[0.75rem]"><span className="text-destructive line-through opacity-70">{o.old}</span><span className="mx-1 text-foreground-subtlest">→</span><span className="text-foreground">{o.new}</span></span>
                        {frames.length > 0 && <span className="text-[0.625rem] text-foreground-subtlest">{frames.length} frame(s) ▸</span>}
                      </summary>
                      {dv && frames.length > 0 && (
                ...
                {sames.length > 0 && (
                  <details className="py-0.5">
                    <summary className="cursor-pointer list-none text-[0.75rem] text-foreground-subtlest">· {sames.length} unchanged ▸</summary>
                    <div className="mt-0.5 flex flex-col gap-0.5">
                      {sames.map((o, i) => (
                        <div key={i} className="flex flex-wrap items-baseline gap-x-2 text-[0.75rem] text-foreground-subtle">
                          <span>{o.label ?? JSON.stringify(o.input).slice(0, 60)}</span>
                          <span className="font-mono text-[0.6875rem] text-foreground-subtlest">{o.new}</span>
                        </div>
                      ))}
                    </div>
                  </details>
