...
function OperandChip({ o, onVary }: { o: Operand; onVary?: (o: Operand) => void }): React.JSX.Element {
  return (
    <span
      className={`inline-flex cursor-default items-center gap-1 rounded border px-1.5 py-0.5 text-[0.75rem] ${provColor[o.provenance] ?? provColor.unknown} ${onVary ? 'cursor-pointer hover:brightness-125' : ''}`}
  ...
  return (
    <div className={`rounded-lg border bg-panel p-3 ${node.business_meaningful ? 'border-border' : 'border-border/50'} ${ghosted ? 'opacity-40 border-dashed' : ''}`}>
      <div className="mb-1.5 flex items-center gap-2">
        <span className="text-[0.625rem] uppercase font-medium text-foreground-subtlest">{node.kind}</span>
        <span className="text-[0.8125rem] font-medium text-foreground">{node.label}</span>
        {ghosted && <span className="rounded-full border border-border px-1.5 text-[0.625rem] text-foreground-subtlest">not observed</span>}
        {node.evidence && <button className="ml-auto text-[0.6875rem] text-foreground-subtlest underline hover:text-foreground" onClick={() => setShowEvidence((s) => !s)}>{showEvidence ? 'hide' : 'output'}</button>}
      </div>
      ...
      {node.question && <div className="mt-1 text-[0.8125rem] text-foreground-subtle">? {node.question}</div>}
      {showEvidence && node.evidence && <pre className="mt-2 max-h-[100px] overflow-auto whitespace-pre-wrap font-mono text-[0.75rem] text-foreground-subtle">{node.evidence}</pre>}
    </div>
  )
}
...
export function DataflowGraph({ graph, onVary, varying }: { graph: GraphModel; onVary?: (o: Operand) => void; varying?: boolean }): React.JSX.Element {
  ...
  const flushRun = (key: string): void => {
    if (run === 0) return
    rows.push(<button key={`p${key}`} className="self-start rounded-md border border-border px-2.5 py-1 text-[0.75rem] text-foreground-subtlest hover:bg-surface-hover" onClick={() => setExpandedPlumbing(true)}>⋯ {run} plumbing step{run === 1 ? '' : 's'} — expand</button>)
    run = 0
  }
  ...
  return (
    <div className={`flex h-full flex-col gap-3 overflow-y-auto p-5 ${varying ? 'opacity-60' : ''}`}>
      <div className="text-[0.625rem] font-semibold uppercase tracking-wide text-foreground-subtlest">observed dataflow</div>
      <h1 className="font-mono text-[1rem] font-semibold text-foreground">{graph.entrypoint}</h1>
      ...
      <div className="flex flex-wrap gap-3 text-[0.6875rem] text-foreground-subtlest">
      ...
      {onVary && <div className="text-[0.75rem] text-foreground-subtle">Click an input value{varying ? ' — fuzzing…' : ''} to fuzz it.</div>}
      ...
      <div className="mt-3 rounded-lg border border-border bg-surface p-3">
        ...
        {graph.questions.length > 0 ? (
          <>
            <h2 className="mt-1 text-[0.875rem] font-semibold text-foreground">These are questions, not findings.</h2>
            <ul className="mt-1 flex flex-col gap-1">{graph.questions.map((q, i) => (<li key={i} className="flex gap-2 text-[0.8125rem] text-foreground-subtle"><span className="text-foreground-subtlest">?</span><span>{q}</span></li>))}</ul>
          </>
        ) : <p className="mt-1 text-[0.75rem] text-foreground-subtlest">No open question for this input. That is not a pass — it is one observed path.</p>}
      </div>
      <div className="mt-auto pt-3 text-[0.75rem] leading-relaxed text-foreground-subtlest">Every value above is measured from one real execution or labelled <i>you supply</i>. grasp surfaces what the code did; whether it is intended is yours to say.</div>
    </div>
  )
}
