...
export function RemoteConnect(props: {
  ...
  const field = 'w-full rounded-lg border border-border bg-input px-3.5 py-2.5 text-[14px] text-foreground outline-none transition-colors placeholder:text-foreground-subtlest focus:border-border-hover'
  const label = 'mb-1.5 block text-[12px] font-medium text-foreground-subtle'
      ...
      <div className="flex w-[480px] max-w-[90vw] flex-col gap-4 rounded-2xl border border-border bg-card p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-foreground-subtlest">The agent, editor, and terminal will run on the remote machine.</p>
          </div>
          <button className="border-0 bg-transparent text-[16px] text-foreground-subtlest transition-colors hover:text-foreground" onClick={props.onClose}>✕</button>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {/* auth mode toggle */}
          <div className="flex gap-1 self-start rounded-lg border border-border bg-background p-0.5 text-[12px]">
          ...
          {auth === 'key' ? (
            <>
              <div>
                ...
                <input className={`${field} font-mono text-[13px]`} placeholder="~/.ssh/id_ed25519" value={keyPath} onChange={(e) => setKeyPath(e.target.value)} spellCheck={false} />
              </div>
              <div>
        ...
        <div className="flex items-center justify-end gap-2 pt-1">
          <button className="rounded-lg border border-border bg-secondary px-3.5 py-2 text-[13px] font-medium text-foreground transition-filter hover:brightness-110" onClick={props.onClose}>Cancel</button>
          <button className="rounded-lg bg-primary px-4 py-2 text-[13px] font-semibold text-primary-foreground shadow-sm transition-filter hover:brightness-110 disabled:opacity-50" disabled={busy || !host.trim() || !user.trim()} onClick={() => void connect()}>
