...
function Delta({ d }: { d: OperandDelta }): React.JSX.Element {
  return (
    <div className="flex items-center gap-1.5 font-mono text-[0.75rem]">
      ...
      <span className="text-[0.625rem] text-foreground-subtlest">[{d.provenance}]</span>
    </div>
  )
}
...
function Chip({ o }: { o: Operand }): React.JSX.Element {
  return (
    <span className={`inline-flex items-center gap-1 rounded border px-1.5 py-0.5 text-[0.75rem] ${provColor[o.provenance] ?? provColor.unknown}`}>
  ...
  return (
    <div className={`rounded-lg border border-border bg-panel p-3 ${ghosted ? 'opacity-40 border-dashed' : ''}`}>
      <div className="mb-1.5 flex items-center gap-2">
        ...
        <span className={`rounded px-1.5 text-[0.625rem] uppercase ${statusBadge}`}>{node.status}</span>
      </div>
...
export function DataflowDiff({ diff }: { diff: GraphDiffModel }): React.JSX.Element {
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto p-5">
      <div className="text-[0.625rem] font-semibold uppercase tracking-wide text-foreground-subtlest">dataflow change</div>
      <h1 className="font-mono text-[1rem] font-semibold text-foreground">{diff.entrypoint}</h1>
      ...
      {diff.empty ? (
        <div className="rounded-lg border border-border bg-surface p-3">
          <div className="text-[0.625rem] font-semibold uppercase tracking-wide text-foreground-subtlest">no behavioral change</div>
          <h2 className="mt-1 text-[0.875rem] font-semibold text-foreground">Same dataflow for this input.</h2>
          <p className="mt-1 text-[0.75rem] leading-relaxed text-foreground-subtlest">{diff.honest_message}</p>
        </div>
      ) : (
        <div className="rounded-lg border border-border bg-surface p-3">
          <div className="text-[0.625rem] font-semibold uppercase tracking-wide text-foreground-subtlest">you adjudicate</div>
          <h2 className="mt-1 text-[0.875rem] font-semibold text-foreground">The dataflow changed. Is this what you expected?</h2>
          <ul className="mt-1 flex flex-col gap-1">{diff.questions.map((q, i) => (<li key={i} className="flex gap-2 text-[0.8125rem] text-foreground-subtle"><span className="text-foreground-subtlest">?</span><span>{q}</span></li>))}</ul>
        </div>
      )}
      <div className="mt-auto pt-3 text-[0.75rem] leading-relaxed text-foreground-subtlest">Old and new were each run for real under the same input; the deltas are measured, not inferred.</div>
    </div>
  )
}
