  ...
  return (
    <div className="rounded-lg border border-border bg-panel p-3">
      <div className="mb-1.5 flex items-center gap-2">
        <span className="text-[10px] uppercase font-medium text-foreground-subtlest">{v.kind}</span>
        <span className="text-[13px] font-medium text-foreground">{v.label}.{v.operand}</span>
        <span className="rounded px-1.5 text-[10px] uppercase bg-tag text-foreground">{v.values.length} values</span>
      </div>
      <div className="flex flex-col gap-0.5">
        ...
        {more > 0 && <div className="text-[11px] text-foreground-subtlest">+{more} more</div>}
      </div>
      <div className="mt-1 text-[13px] text-foreground-subtle">? {v.operand} takes {v.values.length} value{v.values.length === 1 ? '' : 's'} — intended?</div>
    </div>
  )
}
...
function Raise({ r }: { r: FuzzRaise }): React.JSX.Element {
  return (
    <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3">
      <div className="mb-1.5 flex items-center gap-2">
        <span className="text-[10px] uppercase font-medium text-destructive">raised</span>
        <span className="text-[13px] font-medium text-foreground">{r.type}</span>
        <span className="rounded bg-destructive/10 px-1.5 text-[10px] uppercase text-destructive">input</span>
      </div>
      {r.message && <div className="font-mono text-[12px] text-destructive">{r.message}</div>}
      <div className="font-mono text-[12px] text-foreground-subtlest">← {repro(r.reproduce_with)}</div>
    </div>
  )
}
...
export function FuzzView({ report }: { report: FuzzReport }): React.JSX.Element {
  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto p-5">
      <div className="text-[10px] font-semibold uppercase tracking-wide text-foreground-subtlest">fuzzed · {report.ran}/{report.variant_count} inputs</div>
      <h1 className="font-mono text-[16px] font-semibold text-foreground">{report.entrypoint}</h1>
      ...
      <div className="flex flex-col gap-2">
        ...
        {clean && <div className="py-4 text-[13px] text-foreground-subtlest">Nothing varied or raised across {report.ran} inputs. That is not a pass — it is what these seeded inputs exercised{nGaps ? `; ${nGaps} could not be observed.` : '.'}</div>}
      </div>
      {!clean && (
        <div className="rounded-lg border border-border bg-surface p-3">
          <div className="text-[10px] font-semibold uppercase tracking-wide text-foreground-subtlest">you adjudicate</div>
          <h2 className="mt-1 text-[14px] font-semibold text-foreground">Some inputs bend the behavior.</h2>
          <ul className="mt-1 flex flex-col gap-1">{questions.map((q, i) => (<li key={i} className="flex gap-2 text-[13px] text-foreground-subtle"><span className="text-foreground-subtlest">?</span><span>{q}</span></li>))}</ul>
        </div>
      )}
      <div className="mt-auto pt-3 text-[12px] leading-relaxed text-foreground-subtlest">Each value and raise above is measured from a real run of a seeded input; the reproducing input is exact.</div>
    </div>
  )
}
